...
public class CardMove : MonoBehaviour
{
    ...
    // 在 CardMove 类中添加事件声明
    //public event Action<int> OnCardSelected; // 卡牌选中事件（携带序号）

    // 在 CardMove 类顶部添加事件声明
    public event Action<int> OnCardHovered;   // 悬停时触发
    public event Action<int> OnCardUnhovered; // 离开时触发（可选）
    ...
    void AddUIComponents(GameObject go, int idx)
    {
        ...
        click.callback.AddListener(_ => OnCardClicked(idx)); // 传递当前卡牌索引
    ...
    void OnHover(int i, bool over)
    {
        ...
        if (over)
        {
            ...
            OnCardHovered?.Invoke(i); // 触发悬停事件
            c.rect.GetChild(0).gameObject.SetActive(true);
        }
        else
        {
            c.rect.SetSiblingIndex(i);
            c.rect.GetComponent<Canvas>().sortingOrder = cards.Length - i;
            OnCardUnhovered?.Invoke(i); // 可选：触发离开事件
            c.rect.GetChild(0).gameObject.SetActive(false);
        }
    ...
    {
        OnCardSelected?.Invoke(index); // 触发事件并传递序号
    }
    */
}
